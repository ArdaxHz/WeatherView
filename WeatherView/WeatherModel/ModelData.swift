...
class ModelData: ObservableObject {
    ...
    func loadData(lat: Double, lon: Double) async throws -> Forecast {
        let url = URL(
          string: "https://api.openweathermap.org/data/3.0/onecall?lat=\(lat)&lon=\(lon)&units=metric&appid=\(API_KEY)")
    ...
    func getAirPolutionData(lat: Double, lon: Double) async throws -> AirPollution {
        let url = URL(
          string: "https://api.openweathermap.org/data/2.5/air_pollution?lat=\(lat)&lon=\(lon)&appid=\(API_KEY)")
        let session = URLSession(configuration: .default)

        let (data, _) = try await session.data(from: url!)

        do {
            let airPollution = try JSONDecoder().decode(AirPollution.self, from: data)
            DispatchQueue.main.async {
